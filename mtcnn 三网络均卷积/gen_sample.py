...
IMAGE_PATH = r'/media/tensorflow01/myfile/img_celeba'
    ...
    for (i, lebal), (epoch, landmarks) in zip(enumerate(open(LABEL_FILE_PATH).readlines()),enumerate(open(LANDMARTS_FILE_PATH).readlines())):
        ...
        if i > 1 and i< 202599+2:                              #i>1    图片标签从第三行开始.试验7张图片
            ...
            for _ in range(2):               #主要生成正样本
                ...
                _x1 = _cx - side_len/2                #生成浮动框坐标计算
                _y1 = _cy - side_len/2
                _x2 = _x1 + side_len
                _y2 = _y1 + side_len
                                                         #关键点偏移计算错误?

                _lefteye_x = (lefteye_x - _x1 )/side_len #相对坐标 ，关键点不用偏移，直接输出预测关键点相对坐标?
                                                         #此种方式不够泛化，部分样本不能参与训练，效果更差的原因是只用正样本训练。。。
                # _lefteye_x = ()                        #想当然的以为
                _lefteye_y = (lefteye_y - _y1)/side_len  #理解有问题，关键点回归和检测框回归是类似的，原理是一样的，所以也应该用偏移去回归！！！
            ...
            for _ in range(3):               #主要生成部分样本
                ...
                # _lefteye_x = w*dx/side_len   #反算坐标如何，建议关键点在哪呢？
                _lefteye_x = (lefteye_x - _x1) / side_len
...
if __name__ == '__main__':
    ...
    gen_sample(12)
    gen_sample(24)
    gen_sample(48)  #关键点训练只需要生成48*48，只需O网络训练输出关键点  randmarks
