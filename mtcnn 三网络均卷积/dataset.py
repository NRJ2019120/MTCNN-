...
class FaceDataset(Dataset):
    def __init__(self,path,islandmark=True):

        self.islandmark = islandmark
        self.path = path
        self.dataset = []
        self.dataset.extend(open(os.path.join(path,"positive.txt")).readlines()) #列表增添,正 ,负,部分样本标签聚集在dataset中
...
if __name__ == '__main__':
    ...
    root_path_48 =r'/media/tensorflow01/myfile/landmarks_img_celeba/48'
    dataset_48 = FaceDataset(root_path_48,islandmark=True)
    ...
    dataloader = data.DataLoader(dataset_48,batch_size=4,shuffle=True)
    for epoch,(imgdata,con,offset,landmaeks) in enumerate(dataloader):
        print("epoch==>", epoch, "imgdata==>", imgdata, "con==>", con, "offset==>", offset,"landmarks==>",landmaeks)
